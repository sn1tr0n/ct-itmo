...
\begin{enumerate}
    ...
    \[\hat{f}(y) = \frac{1}{a}f_{\frac{1}{a}}(a) = e^{\frac{-\pi y^2}{a^2}}\] 
...
Первое обуславливается $\tilde{C}^{\infty} \subset L^{\infty}$ (мы доказывали что при свёртке функции из $L^1$ c $L^p$, свёртка будет и $L^p$. Почему из этого следует непрерывность я пока не понял, разве нам гаратнируется, что свёртка будет лежать в $\tilde{C}^{\infty}[-\pi, \pi]$?). Ну и дифференцируемость по Лейбницу:
...
Мы оценили сверху, и упихали все константы в $C_1$ ($n$ у нас фиксировано на момент исследования, проверяется $p$). Теперь вспоминаем оценочное неравенство для интеграла по дополнению шара (вроде бы мы доказывали что-то подобное во 2м семе, только на $\mathbb{R}$, ну идея то простая --- за пределпми шара числа большие, а дробь маленькая):
