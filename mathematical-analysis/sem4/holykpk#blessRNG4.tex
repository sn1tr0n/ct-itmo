...
Ну и всё, значит, по теореме Леви можем перейти к предельному преходу интегралов: 
...
Рассмотрим любой замкнутный путь (петлю) $\gamma$. Утверждается, что она гомотопна (петельно) любому постоянному пути $\tilde{\gamma}$ --- по односвязности области. Тогда, можно запустить теорему о равенстве интегралов по гомотопным путям (но там были необходимы \textbf{связанно гомотопные} пути, а у нас они гомотопны петельно. Или пофигу? \textit{Ответ КПК: именно! Мы доказывали всё для связанных, а тут применили для петельных. Так и задумана, произошла подмена. Типа для петельных в теоремах примерно то же самое, поэтому мы для них не доказывали.}). Заметим, что $\tilde{\gamma}'(t) = 0$, так как у постоянного пути скорость нулевая:
...
\[ = \iint_{\Omega} (Px'_v)'_u - (Px'_v)'_v dudv = \iint_{\Omega} (\uwave{P'_xx'_u} + P'_yy'_u+ P'_zz'_u)x'_v + \uwave{P x''_{vu}} - (\uwave{P'_xx'_v }+ P'_yy'_v+ P'_zz'_v)x'_u - \uwave{P x''_{uv}} du dv = \]

Выделенное сокращается, остальное запихивается в определители:

\[ = \iint_{\Omega} P'_yy'_ux'_v+ P'_zz'_ux'_v -P'_yy'_vx'_u -P'_zz'_vx'_u du =\]

\[ = \iint_{\Omega} P'_z \begin{vmatrix}
    z'_u & z'_v \\
    x'_u & x'_v
...
\[ = \frac{1}{a^m} \int_{\mathbb{R}^m}f(\tilde{x})e^{-2 \pi i \sk{\frac{y}{a}}{\tilde{x}}}\frac{d\tilde{x}}{a} = \frac{1}{a^m}\hat{f}\left(\frac{x}{a}\right)\]
